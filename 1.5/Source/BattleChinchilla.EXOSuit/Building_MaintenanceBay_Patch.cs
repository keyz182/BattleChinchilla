...
public static class Building_MaintenanceBay_Patch
{
    ...
    public static void ModuleStorage_Getter_Patch(Building_MaintenanceBay __instance, ref List<Apparel> __result)
    {
        ...
        {
            __result.RemoveAll(app => app is WalkerGear.WalkerGear_Core && app.def != BattleChinchillaDefOf.BC_ChinchillaBattleArmour);
        }
        else
        {
            __result.RemoveAll(app => app.def == BattleChinchillaDefOf.BC_ChinchillaBattleArmour);
        }
    }
